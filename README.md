...
作成した動画はアプリ内でプレビューでき、開始・終了時間を指定してカット版を保存できます。既存のMP4、MOV、M4V、AVI、MKV、WebM、WMV動画をインポートして同じカット機能を使うこともできます。元動画の幅・高さ・回転を解析し、16:9（1920×1080）または9:16（1080×1920）へ比率変換できます。中身の画像は「全体を表示」または中央を拡大する「枠いっぱい」を外枠比率とは別に選べます。さらに任意の画像を、小・中・大・全面のサイズと5か所の位置から選んで動画へ重ねられます。重ねる範囲の元動画は、そのまま残すほか白または黒で隠せます。どちらも元画像の縦横比を崩しません。ダウンロード前には、開始位置から最大10秒の高速な変換プレビューで、カット・比率・重ねる画像・白黒の下地を含む仕上がりを確認できます。出力は互換性の高いH.264/AACのMP4です。
...
- `QuickVideoMaker-Portable-1.2.0.exe`
- `QuickVideoMaker-Portable-1.2.0.exe.sha256`
- `QuickVideoMaker-Portable-1.2.0.zip`
- `QuickVideoMaker-Portable-1.2.0.zip.sha256`
