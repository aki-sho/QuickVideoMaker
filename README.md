...
作成した動画はアプリ内でプレビューでき、開始・終了時間を指定してカット版を保存できます。既存のMP4、MOV、M4V、AVI、MKV、WebM、WMV動画をインポートして同じカット機能を使うこともできます。元動画の幅・高さ・回転を解析し、16:9（1920×1080）または9:16（1080×1920）へ比率変換できます。中身の画像は「全体を表示」または中央を拡大する「枠いっぱい」を外枠比率とは別に選べます。どちらも元画像の縦横比を崩しません。出力は互換性の高いH.264/AACのMP4です。
